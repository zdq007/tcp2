...
func NewSession(protocolGenerator ProtocolGenerator, conn *net.TCPConn) *Session {
	...
	session.sid = fmt.Sprintf("%d",&session)
...
func (self *Session) IsSet(key interface{}) bool{
...
func (self *Session) ClearTimeout() {
	self.conn.SetReadDeadline(time.Time{})
}
//设置心跳超时时间
func (self *Session) Heart() {
	if  self.htw!=nil{
		self.htw.in(self)
	}
}
//设置心跳超时时间
func (self *Session) ClearHeart() {
	if  self.htw!=nil{
		self.htw.out(self)
	}
}
